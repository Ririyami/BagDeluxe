...
if(isset($_POST['add_product'])){
   ...
   if($select_products->rowCount() > 0){
      ...
      if($image_size > 2000000){
         ...
         $insert_product = $conn->prepare("INSERT INTO `products`(name, category, bag_color, price, image) VALUES(?,?,?,?,?)");
         $insert_product->execute([$name, $category, $bag_color, $price, $image]);

         $product_id = $conn->lastInsertId();

         // Insert stock into inventory table
         $insert_inventory = $conn->prepare("INSERT INTO `inventory`(product_id, stock_quantity) VALUES(?,?)");
         $insert_inventory->execute([$product_id, $stock_quantity]);
...
<section class="add-products">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <input type="number" min="0" required placeholder="enter stock quantity" name="stock_quantity" class="box">
